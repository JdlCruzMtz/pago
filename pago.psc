Algoritmo sin_titulo
		...
		Definir impuestoIMSS, impuestoISR, cuotaSindical Como Real
		...
		Si horasExtras <= 8 Entonces
			pagoHorasExtras <- horasExtras * (salarioBase / 7 / 8) * 2 
		SiNo
			pagoHorasExtras <- 8 * (salarioBase / 7 / 8) * 2 
			pagoHorasExtras <- pagoHorasExtras + (horasExtras - 8) * (salarioBase / 7 / 8) * 3 
		Fin Si
